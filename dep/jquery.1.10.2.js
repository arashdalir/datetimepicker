...
(
	function (window, undefined){
		...
		jQuery.each(
			"Boolean Number String Function Array Date RegExp Object Error".split(" "), function (i, name){
		...
		(
			function (window, undefined){
				...
				setDocument = Sizzle.setDocument = function (node){
					...
					support.getElementsByClassName = assert(
						function (div){
						...
						"text": function (elem){
							...
							return elem.nodeName.toLowerCase() === "input" &&
								...
								(
									...
									) == null || attr.toLowerCase() === elem.type
								);
						},
						...
						function (seed, results, context, xml){
							...
							if(postFilter)
							{
								...
								while(i--)
								{
									...
									{
										matcherOut[postMap[i]] = !(
											matcherIn[postMap[i]] = elem
										);
									}
								}
							}
		...
		function createOptions(options)
		{
			...
			jQuery.each(
				options.match(core_rnotwhite) || [], function (_, flag){
					object[flag] = true;
				}
			);
			return object;
			...
			function (support){
				...
				jQuery(
					function (){
						...
						jQuery.swap(
							body, body.style.zoom != null? {zoom: 1}:{}, function (){
		...
		jQuery.extend(
			{
				...
				_queueHooks: function (elem, type){
					...
					return jQuery._data(elem, key) || jQuery._data(
						elem, key, {
					...
					select: {
						...
						set: function (elem, value){
							...
							while(i--)
							{
								option = options[i];
								if((
									option.selected = jQuery.inArray(jQuery(option)
										.val(), values) >= 0
								))
								{
									optionSet = true;
		...
		jQuery.each(
			jQuery.expr.match.bool.source.match(/\w+/g), function (i, name){
			...
			["radio", "checkbox"], function (){
				jQuery.valHooks[this] = {
					set: function (elem, value){
						...
						{
							return (
								elem.checked = jQuery.inArray(jQuery(elem)
									.val(), value) >= 0
							);
						}
					}
		...
		if(!jQuery.support.changeBubbles)
		{
			...
			jQuery.event.special.change = {
				...
				setup: function (){
					...
					jQuery.event.add(
						this, "beforeactivate._change", function (e){
							var elem = e.target;
		...
		jQuery.extend(
			{
				...
				dir: function (elem, dir, until){
					...
					while(cur && cur.nodeType !== 9 && (
						until === undefined || cur.nodeType !== 1 || !jQuery(cur)
							.is(until)
					))
					{
						if(cur.nodeType === 1)
		...
		function winnow(elements, qualifier, not)
		{
			...
			return jQuery.grep(
				elements, function (elem){
					return (
						jQuery.inArray(elem, qualifier) >= 0
					) !== not;
				}
			);
		}
		...
		{
			elem.type = (
				jQuery.find.attr(elem, "type") !== null
			) + "/" + elem.type;
			return elem;
		}
				...
				style: function (elem, name, value, extra){
					...
					if(value !== undefined)
					{
						...
						))
						{
							value = (
								ret[1] + 1
							) * ret[2] + parseFloat(jQuery.css(elem, name));
							// Fixes bug #9237
							type = "number";
						}
		...
		function inspectPrefiltersOrTransports(structure, options, originalOptions, jqXHR)
		{
			...
			function inspect(dataType)
			{
				...
				jQuery.each(
					structure[dataType] || [], function (_, prefilterOrFactory){
		...
		jQuery.each(
			["ajaxStart", "ajaxStop", "ajaxComplete", "ajaxError", "ajaxSuccess", "ajaxSend"], function (i, type){
		...
		jQuery.support.cors = !!xhrSupported && (
		...
		jQuery.offset = {
			...
			setOffset: function (elem, options, i){
				...
				{
					props.top = (
						options.top - curOffset.top
					) + curTop;
				}
				if(options.left != null)
				{
					props.left = (
						options.left - curOffset.left
					) + curLeft;
				}
		...
		jQuery.each(
			{scrollLeft: "pageXOffset", scrollTop: "pageYOffset"}, function (method, prop){
				var top = /Y/.test(prop);
		...
		jQuery.each(
			{Height: "height", Width: "width"}, function (name, type){
				jQuery.each(
					{padding: "inner" + name, content: type, "": "outer" + name}, function (defaultExtra, funcName){
